...
const Tasks = () => {
  ...
  const {
    ...
    error: errorTasks,
    data: dataTasks
  } = useQuery(['tasksData'], () => fetchTasks(query), {
  ...
  const {
    ...
    error: errorLayouts,
    data: dataLayouts
  } = useQuery(['layoutsOnLocalizationData'], () => fetchLayoutsOnLocalization(query), {
    enabled: router.isReady
  });
  ...
  const mutation = useMutation(createTask, {
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(['tasksData']);
  ...
  const assignCoordToTask = (coordId) => {
    ...
    mutationCoord.mutate({
      ids: { clientId, projectId, localizationId, coordId, layoutId },
      selectedTask
    });
  };
  ...
  const onSubmit: SubmitHandler<InputsNewTask> = async (dataToSend) => {
    ...
    const res = mutation.mutate({ ...dataToSend, clientId, projectId, localizationId });
  };
  ...
  return (
    <Layout>
      <div className="px-4 sm:px-6 lg:px-8 w-full">
                    ...
                    {dataLayouts && dataLayouts.length ? (
                      ...
                      <div className="flex flex-col justify-center items-center h-full">
                        ...
                        <UploadLayoutButton
                          ...
                          dataQuery={'layoutsOnLocalizationData'}
                        />
                      </div>
                    )}
                  </div>
        ...
        {selectedTask && <TaskTable data={filterSelectedTask()} />}
      </div>
